...
test("возраст потока считается от входа в обязательство, а не от текущей колонки", () => {
  const card = {
    status: "progress",
    size: "M",
  ...
  assert.equal(getCardAgeDays(card, TEST_NOW), 1);
  assert.equal(getFlowAgeDays(card, TEST_NOW), 6);
});

test("SLE задачи зависит от размера карточки", () => {
  assert.equal(getExpectedCycleDays({ size: "S" }), 2);
  assert.equal(getExpectedCycleDays({ size: "M" }), 4);
  assert.equal(getExpectedCycleDays({ size: "L" }), 7);
});
...
test("формула риска использует опубликованные коэффициенты", () => {
  ...
  const ageRatio = insight.flowAgeDays / insight.expectedDays;
  const expected =
    Math.round(Math.min(ageRatio, SCORING_RULES.ageRatio.cap) * SCORING_RULES.ageRatio.weight) +
...
test("одинаковый возраст рискованнее для маленькой задачи, чем для большой", () => {
  const cards = [
    {
      id: "small",
      title: "Маленькая задача",
      description: "",
      status: "progress",
      priority: "medium",
      size: "S",
  ...
  assert.equal(rankFocusCards(cards, TEST_NOW, 2)[0].card.id, "small");
});
...
test("перемещение карточки сбрасывает enteredAt и снимает блокировку в готово", () => {
  ...
  assert.equal(card.finishedAt, TEST_NOW);
});

test("перенос из бэклога запускает отсчет SLE", () => {
  const moved = moveCard(fixtureCards(), "card-recurring-reset", "ready", TEST_NOW);
  const card = moved.find((item) => item.id === "card-recurring-reset");

  assert.equal(card.status, "ready");
  assert.equal(card.startedAt, TEST_NOW);
});
...
test("новая карточка создается без служебных текстов", () => {
  ...
  assert.equal(card.createdAt, TEST_NOW);
  assert.equal(card.startedAt, TEST_NOW);
