...
function appMarkup() {
  ...
  return `
    ...
    <div class="kb-toolbar">
      ...
      <button class="kb-add-btn" id="addTagBtn" type="button" title="Добавить тег">+ Тег</button>
...
function tagModalMarkup() {
  return `
    <div class="kb-overlay open" id="tagModalOverlay">
      <div class="kb-modal tag-modal">
        <div class="tag-modal-header">
          ...
          <div>
            <div class="kb-modal-title" style="margin-bottom:2px">Новый тег</div>
            <div class="tag-modal-subtitle">Тег появится в фильтрах и при создании задачи</div>
          </div>
        </div>
        ...
        <div class="tag-input-wrap">
          ...
          <div class="tag-preview" id="tagPreview">
            <span class="kb-card-tag tag-custom" id="tagPreviewChip">PREVIEW</span>
          </div>
        </div>

        <div class="tag-color-section">
          <div class="tag-color-label">Цвет тега</div>
          <div class="tag-color-swatches" id="tagColorSwatches">
            ${TAG_COLORS.map((c, i) => `
              <button type="button" class="tag-swatch${i === 0 ? " selected" : ""}" data-color-idx="${i}"
                style="background:${c.bg}; color:${c.text}; border:2px solid ${i === 0 ? c.text : "transparent"}"
                title="${c.name}">
              </button>
            `).join("")}
          </div>
        ...
        <div class="tag-existing-section">
          <div class="tag-color-label">Существующие теги</div>
          <div class="tag-existing-list">
            ${TAGS.map((tag) => `<span class="kb-card-tag tag-${tag}" style="${getTagStyle(tag)}">${tagLabel(tag)}</span>`).join("")}
          </div>
        </div>

        <div class="kb-modal-actions" style="margin-top:20px">
          <button class="kb-modal-cancel" id="tagModalCancel" type="button">Отмена</button>
          <button class="kb-modal-save" id="tagModalSave" type="button">Добавить тег</button>
        </div>
      </div>
    </div>
  `;
...
// Tag color palette
const TAG_COLORS = [
  { name: "Синий",     bg: "#E6F1FB", text: "#0C447C" },
  { name: "Индиго",    bg: "#EEF2FF", text: "#3730A3" },
  { name: "Фиолетовый",bg: "#F5F3FF", text: "#5B21B6" },
  { name: "Розовый",   bg: "#FDF2F8", text: "#9D174D" },
  { name: "Красный",   bg: "#FEF2F2", text: "#991B1B" },
  { name: "Оранжевый", bg: "#FFF7ED", text: "#9A3412" },
  { name: "Жёлтый",   bg: "#FEFCE8", text: "#713F12" },
  { name: "Зелёный",   bg: "#F0FDF4", text: "#14532D" },
  { name: "Циан",      bg: "#ECFEFF", text: "#164E63" },
  { name: "Тёмный",    bg: "#1E293B", text: "#E2E8F0" },
];

// Runtime map: tagName -> color index
const tagColorMap = JSON.parse(localStorage.getItem("tagColorMap") || "{}");
...
function bindAppEvents() {
  // FIX: use { once: true } equivalent by cloning — instead we bind directly on
  // fresh DOM nodes (render() replaces innerHTML each time, so these are always new elements)
  ...
  // FIX: delegate click/submit/change on `app` only once via a single persistent listener.
  // We remove the old one (if any) and add a new one to avoid duplicates.
...
function bindTagModal() {
  ...
  const input = document.querySelector("#tagNameInput");
  const previewChip = document.querySelector("#tagPreviewChip");
  const swatches = document.querySelectorAll(".tag-swatch");
  const saveBtn = document.querySelector("#tagModalSave");
  const cancelBtn = document.querySelector("#tagModalCancel");
  const overlay = document.querySelector("#tagModalOverlay");

  function updatePreview() {
    const val = input.value.trim() || "PREVIEW";
    previewChip.textContent = val.toUpperCase();
    const c = TAG_COLORS[selectedColorIdx];
    previewChip.style.background = c.bg;
    previewChip.style.color = c.text;
  }
  ...
  swatches.forEach((swatch) => {
    swatch.addEventListener("click", () => {
      swatches.forEach((s) => {
        const idx = Number(s.dataset.colorIdx);
        const c = TAG_COLORS[idx];
        s.style.border = `2px solid transparent`;
      });
      selectedColorIdx = Number(swatch.dataset.colorIdx);
      const c = TAG_COLORS[selectedColorIdx];
      swatch.style.border = `2px solid ${c.text}`;
      updatePreview();
    });
  });
